...
class BaselineCareStatusFormValidator(FormValidator):
    ...
    def validate_clinic_access(self, key):
        ...
        self.applicable_if(
            YES, field=f"attending_{key}_clinic", field_applicable=f"use_{key}_clinic_nearby"
        )
        self.required_if(
            NO, field=f"use_{key}_clinic_nearby", field_required=f"{key}_clinic_other"
        )
        self.applicable_if(
            NO, field=f"use_{key}_clinic_nearby", field_applicable=f"{key}_willing_to_transfer"
        )
        self.required_if(YES, field=key, field_required=f"{key}_next_appt_date")

    def validate_attending_clinic(self, key):
        if key == "hiv":
            self.applicable_if(YES, field=key, field_applicable=f"attending_{key}_clinic")
        elif key == "ncd":
            if (
