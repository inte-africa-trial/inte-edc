...
def update_subjectvisit(apps, schema_editor):
    ...
    for obj in subject_visit_model_cls.objects.all():
        if (
            ...
            and not obj.clinic_services.filter(name=STUDY_DEFINED_TIMEPOINT).exists()
        ):
            obj.clinic_services.add(study_defined_timepoint)
        elif (
            obj.visit_code_sequence != 0
            and obj.clinic_services.filter(name=STUDY_DEFINED_TIMEPOINT).exists()
        ):
            obj.clinic_services.remove(study_defined_timepoint)
